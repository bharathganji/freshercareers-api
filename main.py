...
for channel in req_input:
	...
	if not args['limit_download_to_channel_metadata']:
		...
		for i in data:
			...
			if  "message" in i: 
				# print("----------")
				# arr_temp.append(i.get("message"))
				string = i.get("message") +"\n"+i.get("date")
				# temp_date=i.get("date")
				string = string.splitlines()
				arr_temp.append(string)
				# arr_temp.append(temp_date)
		...
		file_path = f'./{channel}_messages.json'
		print ('> Writing posts data before...')
		# print(data)
		print ('> Writing posts after...')
		obj = json.dumps(
			data,
			ensure_ascii=False,
			separators=(',',':')
